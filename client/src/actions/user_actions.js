...
// cartItems = ids of products, userCart = cart with quantity
export function getCartItems(cartItems, userCart) {
  const request = axios
    .get(`${PRODUCT_SERVER}/articles_by_id?id=${cartItems}&type=array`)
    .then((response) => {
      // inject quantity of products inside response
...
export function removeCartItem(id) {
  const request = axios
    ...
    .then((response) => {
      // server returns cart = contains all data and qty, cartDetail = contains products excluding qty, looping and adding qty
